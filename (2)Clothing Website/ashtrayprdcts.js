...
document.addEventListener('DOMContentLoaded', () => {
    const productDetails = document.getElementById('ashTray-product-details');
    const urlParams = new URLSearchParams(window.location.search);
    const productId = urlParams.get('id');

    // Find the product with the matching ID
    const product = ashTrayPrcts.find(p => p.id.toString() === productId);
    ...
    if (product) {
        ...
        <p class='product-colour'>Color: <span class='light-font'> ${product.colour} </span></p>
        <p class='product-cut'>Dimensions: <span class='light-font'>${product.dimensions}</span></p>
        <p class='product-dupatta'>Material: <span class='light-font'>${product.Material}</span></p>
        </div>

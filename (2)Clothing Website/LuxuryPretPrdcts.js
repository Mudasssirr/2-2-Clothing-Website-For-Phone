...
document.addEventListener('DOMContentLoaded', () => {
    const productDetails = document.getElementById('Luxury-Pret-product-details');
    const urlParams = new URLSearchParams(window.location.search);
    const productId = urlParams.get('id');

    // Find the product with the matching ID
    const product = LuxuryPretproducts.find(p => p.id.toString() === productId);
    ...
    if (product) {
        ...
        <p class='product-cut'>Fabric: <span class='light-font'> ${product.fabric} </span></p>
        </div>

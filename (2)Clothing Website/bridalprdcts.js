...
document.addEventListener('DOMContentLoaded', () => {
    const productDetails = document.getElementById('Bridal-product-details');
    const urlParams = new URLSearchParams(window.location.search);
    const productId = urlParams.get('id');

    // Find the product with the matching ID
    const product = bridalPrcts.find(p => p.id.toString() === productId);
    ...
    if (product) {
        ...
        <p class='product-cut'>Shipping Time: <span class='light-font'> 7-10 Business days </span></p>
        </div>
